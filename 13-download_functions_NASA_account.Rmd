...
library(testthat)
# source functions
# will be replaced with library(NRTAPmodel) once package is published
source("./R_functions/download.R")
source("./R_functions/download_support.R")
source("./R_functions/download_test_support.R")
```
...
The `data_download()` function, as well as a series of processing and calculation functions, are currently in production as part of the NRTAPmodel R package. Production versions of these functions can be accessed and downloaded from the Spatiotemporal Exposures and Toxicology (SET) Group's [NRTAPmodel GitHub repository](https://github.com/Spatiotemporal-Exposures-and-Toxicology/NRTAPmodel).
...
functions_sources <- data.frame(functions, sources)
colnames(function_sources) <- c("Function", "Source")
