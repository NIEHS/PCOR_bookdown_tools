...
The main parameter of interest this data set, other than `$geometry`, is `$Density`, which discretely categorizes each wildfire smoke plume as "Light", "Medium", or "Heavy". Checking its class shows that `$Density` is class character. 
...
Creating a new plot with `smoke_sf_density` shows that one polygon represents each smoke density classification.
