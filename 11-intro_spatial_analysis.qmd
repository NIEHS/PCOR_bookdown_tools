...
Check the coordinate reference systems of `smoke_t` and `conus_t`.
...
Both data sets have native coordinate reference systems which were imported during the `terra::vect()` step. The two data sets, however, have different coordinate reference systems from each other. The area of interest for these exploratory analyses is the coterminous United States, so we can transform `smoke_t` and `conus_t` to the Albers Equal Area projected coordinate system (EPSG code: 5070).
...
```{r}
smoke_t_density <- smoke_t_density[1:nrow(smoke_t_density),
                                   c("Density", "agg_n")]
```
...
The `terra::crop()` function can be used to reduce a set of polygons to an area determined by another polygon(s). In this example, we can crop the `smoke_t_density` polygons to the coterminous United States boundaries.
