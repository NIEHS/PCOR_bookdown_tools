...
ggplot(
  ...
  ggtitle(
    expression("PM"[2.5] * " Measurements at EPA Monitoring Locations (2021)")
  ) +
...
ggplot() +
  ...
  ggtitle(
    expression("Annual Mean PM"[2.5] *
                 " Concentration at EPA Monitoring Locations")
  ) +
...
ggplot(
  ...
  geom_line() +
  ggtitle("Monitors with the Highest and Lowest Annual Mean Concentrations") +
  xlab("Date") +
...
The plot is still difficult to interpre because it lacks geospatial context. The grid lines provide latitude and longitude references, but physical or geopolitical borders can help show where the wildfire smoke plumes are relative to the study area of interest. To provide geospatial context to the wildfire smoke plume polygons, we can the United States state border polygons to the plot

#### 2.1.7 Plot (multiple)

The steps taken to access, download, unzip, and import the United States state border data are the same as those taken for the wildfire smoke plume coverage data. Refer to sections 2.0 Access, download, and unzip, and 2.1.1 Import for detailed descriptions.
...
Plot the coterminous United States boundary.

```{r}
ggplot() +
  geom_sf(data = conus_sf) +
  ggtitle("Coterminous United States Boundaries") +
  theme_pubr() +
  theme(plot.title = element_text(hjust = 0.5)) +
  grids()
```
...
Create a plot which shows the distribution of wildfire smoke plumes over the coterminous United States boundaries.
...
Plot the cropped wildfire smoke plume polygons and the coterminous United States boundaries.
...
Now that the contents of the zip files have been saved on your machine and the data files of interest have been identified, import both the wildfire smoke plume coverage data and the United States state border data with `terra::vect()`.

:::callout-note
See sections 2.0 Access, download, and unzip and 2.1.7 Plot (multiple) for obtaining the wildfire smoke plume coverage and United States state border data sets, respectively.
:::
...
```{r}
smoke_t_density <- smoke_t_density[seq_len(nrow(smoke_t_density)),
                                   c("Density", "agg_n")]
```
...
The `terra::crop()` function can be used to reduce `SpatVector` to an area determined by another `SpatVector`. In this example, we can crop the `smoke_t_density` polygons to the coterminous United States boundaries.
...
Combine the wildfire smoke plume indicator data frame with the the coterminous United States boundaries data.
...
The website URL where the NOAA NARR exists is year-specific, meaning there is a unique URL for each annual data set. For the purpose of these exporatory analyses, air temperature dat from the year 2021 will be used
...
```{r, echo = FALSE, eval = TRUE, results = FALSE}
#### TEST CHUNK ####
test_that("temp_air2m_2021.nc file exists.", {
  files <- list.files(paste0(wd, "narr"))
  expect_in("temp_air2m_2021.nc", files)
})
####################
```
...
Inspect the structure of `temp` to see its class, dimensions, variables, and layer names.
...
```{r, echo = FALSE, eval = TRUE, results = FALSE}
#### TEST CHUNK ####
test_that("temp is a SpatRaster.", {
  expect_equal(class(temp)[1], "SpatRaster")
})
####################
```
...
```{r, echo = FALSE, eval = TRUE, results = FALSE}
#### TEST CHUNK ####
test_that("Dimensions of temp are correct.", {
  expect_equal(nrow(temp), 277)
  expect_equal(ncol(temp), 349)
  expect_equal(nlyr(temp), 365)
})
####################
```
...
`temp` has a native coordinate reference system, but it is unnamed and was not identifiable by `terra::rast()`. The area of interest for these exploratory analyses is the coterminous United States, so we can project `temp` to the Albers Equal Area projected coordinate system (EPSG code: 5070).
...
We want to create plots with and analyze the air temperature data as it relates to the coterminous United States boundaries. Subset `states_t` to remove Alaska, Hawaii, and the United States' territories.
...
Now that the data sets and coordinate reference systems have been prepared, initiate a plot with `ggplot2::gglot()`. Identifying the data sets to be plotted within the `geom_spatraster()` and `geom_spatvector()` argument informs the function that the `temp` and `conus_t` data sets are `SpatRaster` and `SpatVector` objects, respectively.

Only the first layer of the `temp` data set will be plotted.

```{r}
ggplot() +
  geom_spatraster(data = temp$air_20210101) +
...
Plot the cropped temperature data and the coterminous United States boundaries.

Only the first layer of the `temp` data set will be plotted.

```{r}
ggplot() +
  geom_spatraster(data = temp_crop$air_20210101) +
  scale_fill_continuous(
    type = "viridis",
    na.value = NA,
    "Temperature (°K)"
...
The previous plot depicts the 2m air temperature data in Kelvin. To convert the units to degrees Celsius, simply subtract the values in `temp_crop` by 273.15. This subtraction will be applied to every grid cell within every layer of `temp_crop`.

```{r}
temp_crop_c <- temp_crop - 273.15
```

### 3.8 Summary statistics

Similar to mathematical operations, calculating summary statistics is very straight forward with raster data. For this example, calculate the annual mean 2m air temperature and range of 2m air temperatures at each grid cell, and save these as a new layer in `temp_crop_c`

```{r}
temp_crop_c$mean <- mean(temp_crop_c)
temp_crop_c$range <- max(temp_crop_c) - min(temp_crop_c)
```

Inspect the results of the mean and range calculations.

```{r}
summary(temp_crop_c$mean)
summary(temp_crop_c$range)
```

With the summary statistic layers prepared, initiate a plot with `ggplot2::ggplot()`. Identifying the data sets to be plotted within the `geom_spatraster()` and `geom_spatvector()` argument informs the function that the `temp_crop_c` and `conus_t` data sets are `SpatRaster` and `SpatVector` objects, respectively. Additionally, the `facet_wrap(~lyr)` argument creates a plot for each layer specified in `geom_spatraster(data = temp_crop_c[[c("mean", "range")]])`.

```{r, warning=FALSE}
ggplot() +
  geom_spatraster(data = temp_crop_c[[c("mean", "range")]]) +
  scale_fill_continuous(
    type = "viridis",
    na.value = NA,
    "Temperature (°C)"
...
The `terra::zonal()` function can be used to calculate the average annual temperature in each state based on the annual grid cell temperatures stored in `temp_crop_c$mean`.

```{r, warning=FALSE}
conus_t$MEAN <- zonal(temp_crop_c$mean,
  conus_t,
  fun = "mean"
)
```
...
```{r}
temp_reclass <- classify(temp_crop_c$mean,
  rcl = reclass,
  right = TRUE
)
```

Although `temp_reclass` contains the reclassified mean air temperature data, the data is still continuously numeric. The following chunk of code converts the `temp_reclass$mean` layer from numeric to character based on the defined levels.

`temp_reclass` contains the reclassified mean air temperature data, but the data is still numeric. The `terra::set.cats()` function assigns categories to numeric data based on values stored in a data frame.
...
```{r}
ggplot() +
  geom_spatraster(data = temp_reclass$mean_discrete) +
