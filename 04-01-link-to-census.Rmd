# Linkage to Census Units {#link-to-census}
...
**Key Terms**: [Data Integration](https://tools.niehs.nih.gov/cchhglossary/?keyword=data+integration&termOnlySearch=true), [Social Determinants of Health](https://tools.niehs.nih.gov/cchhglossary/?keyword=social+determinants+of+health&termOnlySearch=true), [Geocoded Address](#def-geocoded-address), [Geographic Unit](#def-geographic-unit), [Geoid](#def-geoid)
...
1.   [Install R packages](#step-0)

2.   [Prepare geocoded addresses](#step-1)

3.   [Access Census geographic boundaries](#step-2)

4.   [Link geocoded addresses to Census geographic boundaries](#step-3)

5.   [Link AHRQ SDOH data to geocoded addresses](#step-4)

## Tutorial

### Install R packages {#step-0}
...
### Prepare geocoded addresses {#step-1}
...
### Access Census geographic boundaries {#step-2}
...
### Link geocoded addresses to Census geographic boundaries {#step-3}
...
### Link AHRQ SDOH data to geocoded addresses {#step-4}
...
Based on this information, we can use the geoids for year-2010 vintage Census tracts we prepared for the geocoded addresses in the previous [step](#step-3) to link the AHRQ SDOH data for years 2010-2019. If we would instead prefer to link the AHRQ SDOH data for year 2009 or 2020, we would first need to repeat the previous [step](#step-3) to link the geoids for year-2000 vintage or year-2020 vintage Census tracts, respectively, to the geocoded addresses.
...
We can see that the variable `TRACTFIPS` contains the 11-digit geoid for Census tracts in character format, which matches the geoid format we prepared in the previous [step](#step-3) .
