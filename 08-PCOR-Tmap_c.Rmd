...
```{R}
us_epa_daily_2022_CA <- us_epa_daily_2022[us_epa_daily_2022$State.Name=='California',]
...
# Read in the shapefile (replace 'counties' with the path to your actual shapefile)
...
histogram <-
  ...
  scale_fill_gradientn(colors = aqi_colors, 
                       breaks = aqi_breaks,
                       limits = c(0, 500), 
                       guide = "colorbar",
                       name = "AQI Value")+
...
tm_shape(merged_data_sf_ca, projection = 5070) +
    ...
    tm_fill(col = "Mean_of_Arithmetic_Mean", 
             breaks = aqi_breaks,
             palette = aqi_colors,
             alpha = 0.6) +
    tm_legend(bg.color="white", bg.alpha=0.6)+
    tm_layout(title = "Mean PM2.5",
              attr.color = "navy",
