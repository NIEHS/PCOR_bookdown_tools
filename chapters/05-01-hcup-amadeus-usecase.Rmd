...
Understanding the relationship between environmental exposures and health outcomes is critical for informing public health interventions and policy decisions. Integrating Healthcare Cost and Utilization Project (HCUP) data with environmental exposure datasets allows researchers to examine how factors such as air pollution, wildfire smoke, and extreme heat influence hospital visits and healthcare utilization patterns.

HCUP provides detailed hospitalization records, including diagnoses, procedures, and patient demographics, making it a valuable resource for studying environmentally driven health impacts. By linking these data with environmental exposure metrics—such as air quality indices, satellite-derived pollution estimates, or climate-related hazard data—researchers can identify spatial and temporal trends in health conditions like respiratory diseases, cardiovascular events, and heat-related illnesses.

This integration also provides an opportunity to assess disparities in environmental health burdens. Many environmental hazards disproportionately affect marginalized communities, including low-income populations and communities of color. By combining HCUP data with geographic and socioeconomic indicators, researchers can explore how social determinants of health influence vulnerability to environmental stressors. This approach supports targeted policy interventions aimed at reducing health inequities.

Ultimately, linking HCUP and environmental exposure data enhances public health surveillance, strengthens early warning systems, and informs community-level strategies for mitigating health risks associated with environmental change.
Commit changes
...
years     <-  2017   # 2016:2021
data_type <- 'CORE'  #c("CORE", "DX_PR_GRPS", "CHGS,SEVERITY")

# Define possible data sources
data_sources <- "SEDD" #c("SASD", "SEDD","SID")  
```

```{r, echo=FALSE}
# Missing values definition
missing_values <- as.character(quote(c(-99, -88, -66, -99.9999999, -88.8888888, 
                                       -66.6666666, -9, -8, -6, -5, -9999, -8888,
                                       -6666, -99999999, -999999999, -888888888,
                                       -666666666, -999, -888, -666)))
...
for (data_source in data_sources) {
  ...
  for (year in years) {
    ...
    # Year 2021 had a slightly different format on the specifications at meta_url below
    if (year == 2021) {
    ...
    meta_url <- paste0('https://hcup-us.ahrq.gov/db/state/', data_source_lower_c,
                       '/tools/filespecs/OR_', data_source, '_', year, '_', 
                       data_type, '.loc')
...
#  1 OR     2021 CORE      1 AGE          1     3    NA Num   Age in years at adm…
#  2 OR     2021 CORE      2 AGEDAY       4     6    NA Num   Age in days (when a…
#  3 OR     2021 CORE      3 AGEMONTH     7     9    NA Num   Age in months (when…
...
#  7 OR     2021 CORE      7 AWEEKEND    18    19    NA Num   Admission day is a …
#  8 OR     2021 CORE      8 CPT1        20    24    NA Cha   CPT/HCPCS procedure…
#  9 OR     2021 CORE      9 CPT2        25    29    NA Cha   CPT/HCPCS procedure…
# 10 OR     2021 CORE     10 CPT3        30    34    NA Cha   CPT/HCPCS procedure…
# ℹ 692 more rows
...
temp_covar <- calculate_hms(
  ...
  locs = tigris::zctas(state = "OR", year = 2010), # Use Oregon ZIP code boundaries
