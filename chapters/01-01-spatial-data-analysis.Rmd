...
For detailed descriptions of each type of spatial data, please see [Simple Features for R](https://r-spatial.github.io/sf/articles/sf1.html)\cite{r-sf-article1} for point and polygon data types, and [Introduction to Raster Data](https://datacarpentry.org/organization-geospatial/01-intro-raster-data)\cite{raster-data} for raster data.
...
A coordinate reference system defines how spatial data are positioned relative to the surface of the Earth, a critical piece of conducting spatial analyses (cite geocomputation 2.4)

Projected coordinate systems utilize geographic CRS to depict the CRS on a flat surface (cite).

When transforming (projecting) data to a projected CRS, the data will be distoried in area, direction, distance, or shape.

As the following analyses utilize the United States for spatial context, US-centric coordinate reference system (EPSG Code: 5070) will be used.

For more information on how to select a coordinate reference system for your desired outcome, see [Choose the right projection](https://learn.arcgis.com/en/projects/choose-the-right-projection/). 

## Point Data with `sf`

Air pollution monitoring data from the United States Environmental Protection Agency (EPA) will be used to demonstrate using point data with the `sf` package.\cite{r-sf-1}
...
Now that each monitor location as a unique identification code, we can calculate the mean PM~2.5~ concentration measured at each monitoring location. Functions and syntax from the `dplyr` package will be used to do this.\cite{r-dplyr} For more on the `dplyr` package, please see [Introduction to dplyr](https://dplyr.tidyverse.org/articles/dplyr.html).\cite{dplyr-introduction}
...
This section will focus on exploratory analyses with polygon data using the `terra` package.\cite{r-terra}
...
Now that the parameters of interest and coordinate reference systems have been prepared, create a plot with `ggplot2::ggplot()`. Identifying the data sets to be plotted within the `tidyterra::geom_spatvector()` arguments informs the function that the data are `SpatVector` objects.\cite{r-tidyterra}
