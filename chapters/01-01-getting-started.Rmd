
# Getting Started {#chapter-getting-started-spatial}

[![Profile-CMP](images/user_profiles/profilecmp.svg)](#profilecmp) [![Profile-SBS](images/user_profiles/profilesbs.svg)](#profilesbs) [![Profile-STU](images/user_profiles/profilestu.svg)](#profilestu)

### Getting Started with Geospatial Data Analysis in Environmental Health Using R {-}

**Date Modified:** June 25, 2024

**Authors:** Mitchell Manware [![author-mm](images/orcid.png){width="10"}](https://orcid.org/0009-0003-6440-6106),  Lara P. Clark [![author-lpc](images/orcid.png){width=10}](https://orcid.org/0000-0001-6940-5442), Kyle P. Messier [![author-kpm](images/orcid.png){width="10"}](https://orcid.org/0000-0001-9508-9623)

**Key Terms:** [Environmental Health](https://tools.niehs.nih.gov/cchhglossary/?keyword=environmental+health&termOnlySearch=true&exactSearch=true), [Geospatial Data](#def-geospatial-data)

**Programming Language:** R

## Introduction

### Motivation

Environmental health research relies on various types of data to accurately measure, model, and predict exposures. Environmental data are often spatial (related to the surface of the Earth), temporal (related to a specific time/period of time), or spatio-temporal (related to the surface of the Earth for a specific time/period of time). These data are at the core of environmental health research, but the steps between identifying a geospatial data set or variable and using it to help answer a research question can be challenging.

The chapters in the [Geospatial Data Foundations Unit](#unit-geospatial-foundations) are designed to introduce the necessary steps for conducting analyses with spatial data in R. They will introduce R packages that are equipped to handle spatial data, and will demonstrate how to access, import, and analyze three different types of spatial data. The tutorials will focus primarily on spatial data, but some aspects of temporal and spatio-temporal data will also be discussed.

### Objectives

This chapter will introduce the key concepts and terminology used throughout the CHORDS Toolkit for geospatial data and geospatial analysis methods. Additionally, this chapter will introduce the public geospatial datasets and key `R` packages used in the tutorials in this unit. Please see the [Additional Resources](#chapter-getting-started-spatial-resources) Section for a list of other useful resources for getting started with geospatial data analysis in environmental health using `R`.

In the following chapters in the [Geospatial Data Foundations Unit](#unit-geospatial-foundations), users will learn about the following topics related to spatial data in `R`:

-   Point, polygon, and raster data types
-   Downloading data from a URL
-   Importing data
-   Checking data type, structure, and class
-   Reclassifying data
-   Computing summary and zonal statistics
-   Plotting individual and multiple data sets

## Concepts and Terminology

### Spatial Geometry

There are three spatial geometry types: point, line, and area (i.e., polygon or grid). Uniform geometry data (i.e., grid) is refered to as _raster_ data, and non-uniform geometry data (i.e., point, line, polygon) is refered to as _vector_ data. Please see [Simple Features for R](https://r-spatial.github.io/sf/articles/sf1.html) [@r-sf-1] and [Introduction to Raster Data](https://datacarpentry.org/organization-geospatial/01-intro-raster-data) [@raster-data] for detailed descriptions of the vector and raster data types, respectively. 

The following table illustrates common examples of each spatial geometry type used in environmental health applications:

|Type  |Illustration |Examples                                      |Chapters |
|------|-------------|----------------------------------------------|---------|
|Point (Vector) |![](images/intro_to_spatial/illustration-point.png){width=60%} |Air pollution monitor, Weather station, Patient geocoded address, Healthcare facility coordinates |[Point Data](#chapter-point-data) |
|Line (Vector) |![](images/intro_to_spatial/illustration-line.png){width=60%} |Road network, Travel path | |
|Polygon (Vector)|![](images/intro_to_spatial/illustration-polygon.png){width=60%} |Wildfire smoke plumes, Census boundaries |[Polygon Data](#chapter-polygon-data) |
|Grid (Raster) |![](images/intro_to_spatial/illustration-grid.png){width=60%} |Land cover imagery, Meteorological model output, Gridded population counts |[Raster Data](#chapter-raster-data) |

### Coordinate Reference Systems and Projections {#getting-started-spatial-crs}

Coordinate reference systems (CRS) are important for spatial analyses as they define how spatial data align with the Earth's surface [@geocomputation-24]. Transforming (projecting) the data to a different CRS may be necessary when combining multiple datasets or creating visuals for particular areas of interest. It is important to note that transforming spatial data can cause distortions in its area, direction, distance, or shape [@geocomputation-24]. The direction and magnitude of these distortions vary depending on the chosen CRS, area of interest, and type of data [@article_steinwand1995reprojection]. For guidance on selecting an appropriate CRS based on the data, area of interest, and analysis goals, see [Choose the Right Projection](https://learn.arcgis.com/en/projects/choose-the-right-projection/) [@arcgis-crs].

For the following analyses which focus on the conterminous United States, the Albers Equal Area projection (EPSG Code: 5070) will be utilized.

## Data Sources

The exploratory analyses utilize free and publicly available environmental data. The code chunks are designed to access each specific file used for the exploratory analyses, but a description of each data source and data set is available below.

| Producer | Data | Data Type |
|-----------------|----------------|-------|
| Environmental Protection Agency (EPA) | [PM~2.5~ Daily Observations](https://aqs.epa.gov/aqsweb/airdata/download_files.html) | Point |
| National Oceanic and Atmospheric Administration (NOAA) | [Wildfire Smoke Plumes](https://www.ospo.noaa.gov/Products/land/hms.html) | Polygon |
| United States Census Bureau | [United States Cartographic Boundary](https://www.census.gov/geographies/mapping-files/time-series/geo/carto-boundary-file.html) | Polygon |
| National Oceanic and Atmospheric Administration (NOAA) | [Land Surface Temperature](https://psl.noaa.gov/data/gridded/data.narr.html) | Raster |

: Exploratory analyses data sources

## `R` Packages

Various R packages can be used to create, import, analyze, and export spatial data. If you have not used these packages previously, they may not be installed on your machine. The following chunk of code installs and imports the packages required to conduct the exploratory analyses in this unit.

::: note
Installing and importing new packages may required `R` to restart.
:::

```{r, eval = FALSE}
vignette_packages <- c(
  "dplyr", "ggplot2", "ggpubr", "sf",
  "terra", "tidyterra", "utils"
)

for (v in seq_along(vignette_packages)) {
  if (vignette_packages[v] %in% installed.packages() == FALSE) {
    install.packages(vignette_packages[v])
  }
}

library(dplyr)
library(ggplot2)
library(ggpubr)
library(sf)
library(terra)
library(tidyterra)
library(utils)
```


The `ggplot2` and `ggpubr` packages will be used throughout this unit for creating publication quality plots. Please see [ggplot2: Elegant Graphics for Data Analysis (3e)](https://ggplot2-book.org/introduction) [@r-ggplot2] and [ggpubr: 'ggplot2' Based Publication Ready Plots](https://rpkgs.datanovia.com/ggpubr/) [@r-ggpubr] for in depth descriptions of the syntax and functionality utilized by these packages.

::: important
The exploratory analyses in this unit are designed for educational purposes only. The results of the following analyses are not peer-reviewed findings, nor are they based on any hypotheses.
:::

## Additional Resources {#chapter-getting-started-spatial-resources}

For additional resources pertaining to the packages used in this unit, please see the following:

-   [`sf`](https://cran.r-project.org/web/packages/sf/sf.pdf)

-   [`terra`](https://cran.r-project.org/web/packages/terra/terra.pdf)

-   [`dplyr`](https://cran.r-project.org/web/packages/dplyr/dplyr.pdf)

-   [`ggplot2`](https://cran.r-project.org/web/packages/ggplot2/ggplot2.pdf)

-   [`ggpubr`](https://cran.r-project.org/web/packages/ggpubr/ggpubr.pdf)

-   [`tidyterra`](https://cran.r-project.org/web/packages/tidyterra/tidyterra.pdf)
