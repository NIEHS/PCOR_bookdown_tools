...
From asthma_patients_pm_2\_census_block_group_0.6.0_2010.csv run python biv_cho_map.py call fcc api to get county FIPS and Name for each patient <https://geo.fcc.gov/api/census/#!/block/get_block_find> new data fileasthma_patients_pm_2\_census_block_group_0.6.0_2010.csv
...
brks_pm <- classIntervals(log(ma_counties_data$pm2_mean), n=nbins, style='quantile')

```

```{r}
brks_patient <- classIntervals(log(ma_counties_data$patient_count), n=nbins, style='quantile')
...
```{r}
m4 <- mapplot(rownames(ma_counties_data) ~ class2, data = ma_counties_data,
